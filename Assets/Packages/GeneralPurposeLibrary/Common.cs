...
namespace GPL
{
    public class PIDController
    {
        ...
        /// <summary>
        /// When error accumulation bigger than this value is, 
        /// the integral term will just reset.
        /// </summary>
        public double IntegralReset { get; set; }
        ...
        public PIDController(double kp, double ki, double kd, double integralReset, double maxOutput, double minOutput)
        {
            Kp = kp;
            Ki = ki;
            Kd = kd;
        ...
        /// <summary>
        /// This method takes the sensor readings and the time step since the last call, 
        /// and generates an appropriate input.
        /// </summary>
        /// <param name="currentValue"></param>
        /// <param name="targetValue"></param>
        /// <param name="timeStep"></param>
        /// <returns></returns>
        public double Update(double currentValue, double targetValue, double timeStep)
        {
            double error = targetValue - currentValue;

            integral += error;
            if (Mathf.Abs((float)integral) >= IntegralReset)
                integral = 0f;
            if (error == 0f)
                integral = 0f;
            ...
            double res = error * Kp + integral * Ki + derivative * Kd;
            if (res < MinOutput) res = MinOutput;
            else if (res > MaxOutput) res = MaxOutput;

            return res;
        }

        /// <summary>
        /// Clear the calculations done until now
        /// </summary>
        public void Clear()
        {
            lastError = 0f;
    ...
    /// <summary>
    /// Used to validate if editor components have their values
    /// within the constraints, in order to not cause any 
    /// errors. 
    /// </summary>
    public interface IValidatable
    {
        /// <summary>
        /// Checks if the component that implements this interface
        /// has all the right values or not(values out of constraints)
        /// </summary>
        /// <returns>True if all values are within limits/constraints, otherwise false</returns>
        bool ValidateInput();
    }

    /// <summary>
    /// Boolean that is passed by reference
    /// </summary>
    [Serializable]
     ...
     public static class GUILines
    {
        ...
        public static void DrawBezierLine(Vector2 start, Vector2 startTangent, Vector2 end, Vector2 endTangent, Color color, float width, bool antiAlias, int segments)
        {
            ...
            for (int i = 1; i < segments + 1; ++i)
            {
                ...
                GUILines.DrawLine(lastV, v, color, width, antiAlias);
                lastV = v;
            }
        }
